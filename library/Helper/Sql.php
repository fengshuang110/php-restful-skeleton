...
//		}
	...
	public function limit($start_or_count,$count=null){
		...
		{
			$this->limit_start = $start_or_count;
			$this->limit_end = $count;
		}
		else
		{
			$this->limit_end = $start_or_count;
		}
	...
	public function __toString(){
		switch($this->action){
			...
			case "insert2":return $this->_insert2();
			default: return "";
		}
			...
			foreach($this->select as $alias => $columns){
				foreach((array)$columns as $col_alias=>$column){
					...
					else
					{
						$table = $this->from[$alias];
						foreach($table["fields"] as $col){
							$select[] = "{$alias}.{$col} AS {$alias}_{$col}";
						}						
					}*/
	...
	private function _insert(){
		foreach($this->insert as $alias => $insertDatas){
			foreach((array)$insertDatas as $field => $value){
				...
				$insert_value[] = $this->_quoteValue($filtered['value']);
			}
		}
		$where = $this->_where();
	...
	private function _update(){
		foreach($this->update as $alias => $updateDatas){
			foreach((array)$updateDatas as $field => $value){
				...
				/*$update[] = $alias.".".$field."="
				.(is_numeric($filtered['value']) ? $filtered['value'] : "'".$filtered['value']."'");*/
				$update[] = $alias.".".$field."=" . $this->_quoteValue($filtered['value']);
			}
		}
		$where = $this->_where();
	...
	private function _limit(){
		if($this->limit_start && $this->limit_end){
			...
			return " LIMIT ".(int)$this->limit_start . "," . self::INF;
		}if($this->limit_end){
	...
	private function _quoteValue($value){
		if(is_array($value)){
			...
			{
				$retval = "'" . @mysql_escape_string($value) . "'";
			}
	...
	private function _buildWhere($wheres){
		...
		#处理where中有子查询的情况
		if($wheres['value'] instanceof SqlHelper){
			switch($wheres['op']){
				case self::NOTIN:	
		...
		/*TODO: plugin
		$conds = Plugin::getPlugin()->doFilter("set_value_to_db",array(
		...
		switch($wheres['op']){
			case self::LIKE:		$cond = " LIKE '%".$conds['value']."%'";break;
			case self::NOT_LIKE:		$cond = " NOT LIKE '".$conds['value']."'";break;
			case self::BEFORE_LIKE:	$cond = " LIKE '%".$conds['value']."'";break;
			case self::END_LIKE:	$cond = " LIKE '".$conds['value']."%'";break;
			case self::EQ:	
