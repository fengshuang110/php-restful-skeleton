...
class Third_Yuntongxun {
	...
	public static function getInstance(){
		if(is_null(self::$obj)){
			self::$obj = new Third_Yuntongxun();
		}
		return self::$obj;
	}
	private function __construct() {
		
		$config=  require(LIBRARY_DIR.'/Third/Conf/account.php');
		if(empty($config['yuntongxun'])){
			throw new Exception('can not found the weixin_config  in Third/Conf/account.php');
		};
		$options = $config['yuntongxun'];
...
//        }
    ...
    */
    function voiceVerify($verifyCode,$to,$playTimes=3,$displayNum="",$respUrl="")
    {
        //主帐号鉴权信息验证，对必选参数进行判空。
        $auth=$this->accAuth();
        if($auth!=""){
